/* This example requires Tailwind CSS v2.0+ */
import { MailIcon, PhoneIcon } from '@heroicons/react/solid'
import { BsLinkedin, BsGithub } from "react-icons/bs";
import img1 from "./contact-us-photos/1.jpg";
import img2 from "./contact-us-photos/2.jpg";
import img3 from "./contact-us-photos/3.jpg";
import img4 from "./contact-us-photos/4.jpg";
import img5 from "./contact-us-photos/5.jpg";
const people = [
  {
    name: 'Siladitya Marik',
    title: 'Logo and Posters designing competition',
    role: 'Co-head of IIC Graphics Wing',
    telephone: '7003763872',
    linkedin: '',
    imageUrl: img1
  },
  {
    name: 'Shubhayu Majumdar',
    title: 'Event Head – HackUrWay',
    role: 'Co-Head Technical Wing',
    telephone: '7679325872',
    linkedin: 'https://www.linkedin.com/in/shubhayumajumdar/',
    imageUrl: img2  },
  {
    name: 'Shristi Rai',
    title: 'Event Head – Brain-It-Out',
    role: 'Co-head of PR wing',
    telephone: '7980149344',
    linkedin: 'https://www.linkedin.com/in/shristi-rai-03a264209',
    imageUrl: img3
  },
]

const peopleGen = [
  {
    name: 'Ayushi Dey',
    title: '',
    role: 'Management Head at IIC TMSL',
    telephone: '9641099272',
    linkedin: 'https://www.linkedin.com/in/ayushi-dey-390',
    imageUrl: img4
  },
  {
    name: 'Kushal Guha',
    title: '',
    role: 'Innovation Wing Member',
    telephone: '8337022539',
    linkedin: 'https://www.linkedin.com/in/kushal-guha-8a2983173',
    imageUrl: img5  },
]

export default function Contact() {
  return (
    <div className='flex justify-center pt-32 pb-20'>
      <div className='w-4/5'>
      <h1 id="contact-us"className="text-center text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl pb-20">Contact Us</h1>
           <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {people.map((person) => (
        <li key={person.email} className="col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200">
          <div className="w-full flex items-center justify-between p-6 space-x-6">
            <div className="flex-1 truncate">
              <div className="flex items-center space-x-3">
                <h3 className="text-gray-900 text-sm font-medium">{person.name}</h3>
                <span className="flex-shrink-0 inline-block px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full">
                  {person.role}
                </span>
              </div>
              <p className="mt-1 text-gray-500 text-sm truncate">{person.title}</p>
            </div>
            <img className="w-10 h-10 bg-gray-300 rounded-full flex-shrink-0" src={person.imageUrl} alt="" />
          </div>
          <div>
            <div className="-mt-px flex divide-x divide-gray-200">
              <div className="-ml-px w-0 flex-1 flex">
                <a
                  href={person.linkedin}
                  className="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500"
                >
                  <BsLinkedin className="w-5 h-5 text-gray-400" aria-hidden="true" />
                  <span className="ml-3">LinkedIn</span>
                </a>
              </div>
              <div className="-ml-px w-0 flex-1 flex">
                <a
                  href={`tel:${person.telephone}`}
                  className="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500"
                >
                  <PhoneIcon className="w-5 h-5 text-gray-400" aria-hidden="true" />
                  <span className="ml-3">Phone</span>
                </a>
              </div>
            </div>
          </div>
        </li>
      ))}
    </ul>
    <h1 id="contact-us"className="text-center text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl pt-20 pb-20">General Queries</h1>
    <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2">
      {peopleGen.map((person) => (
        <li key={person.email} className="col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200">
          <div className="w-full flex items-center justify-between p-6 space-x-6">
            <div className="flex-1 truncate">
              <div className="flex items-center space-x-3">
                <h3 className="text-gray-900 text-sm font-medium">{person.name}</h3>
                <span className="flex-shrink-0 inline-block px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full">
                  {person.role}
                </span>
              </div>
              <p className="mt-1 text-gray-500 text-sm truncate">{person.title}</p>
            </div>
            <img className="w-10 h-10 bg-gray-300 rounded-full flex-shrink-0" src={person.imageUrl} alt="" />
          </div>
          <div>
            <div className="-mt-px flex divide-x divide-gray-200">
              <div className="-ml-px w-0 flex-1 flex">
                <a
                  href={person.linkedin}
                  className="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500"
                >
                  <BsLinkedin className="w-5 h-5 text-gray-400" aria-hidden="true" />
                  <span className="ml-3">LinkedIn</span>
                </a>
              </div>
              <div className="-ml-px w-0 flex-1 flex">
                <a
                  href={`tel:${person.telephone}`}
                  className="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500"
                >
                  <PhoneIcon className="w-5 h-5 text-gray-400" aria-hidden="true" />
                  <span className="ml-3">Phone</span>
                </a>
              </div>
            </div>
          </div>
        </li>
      ))}
    </ul>
    <h1 className="text-center text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl pt-20 pb-20">Contact IIC</h1>
    <div className='flex justify-center'>
    <div className="w-auto col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200">
          <div className='p-8 text-center'>
          TECHNO INDIA : EM-4/1, Sector-V, <br/>
        Salt Lake, Kolkata-700091,West Bengal
          </div>
          <div>
            <div className="-mt-px flex divide-x divide-gray-200">
              <div className="-ml-px w-0 flex-1 flex">
                <a
                  href='mailto:iic.tmsl@gmail.com'
                  className="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500"
                >
                  <MailIcon className="w-5 h-5 text-gray-400" aria-hidden="true" />
                  <span className="ml-3">Email</span>
                </a>
              </div>
              <div className="-ml-px w-0 flex-1 flex">
                <a
                  href={`tel:+918271538524`}
                  className="relative w-0 flex-1 inline-flex items-center justify-center py-4 text-sm text-gray-700 font-medium border border-transparent rounded-br-lg hover:text-gray-500"
                >
                  <PhoneIcon className="w-5 h-5 text-gray-400" aria-hidden="true" />
                  <span className="ml-3">Phone</span>
                </a>
              </div>
            </div>
          </div>
        </div>
    </div>
    </div>
    </div>
  )
}
